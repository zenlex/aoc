...
fn main(input: &str, part: usize) -> Result<usize, Box<dyn Error>> {
    ...
    Ok(almanac.get_lowest_location(seeds))
}
...
impl Almanac {
    ...
    fn get_lowest_location(&self, seeds: Seeds) -> usize {
        let mut location = 0;
        while self.get_seed(location, &seeds).is_none() {
            if location % 1000000 == 0 {
                println!("Location: {}", location);
            }
            if location > 24092691 {
                panic!("Location too high");
            }
    ...
    fn reverse_lookup(&self, needle: usize, map: &Map) -> usize {
        for key in map.keys() {
            let (offset, range) = map.get(key).unwrap();
            let offset_needle;
            match offset {
                Offset::Pos(v) => match needle.checked_sub(*v) {
                    Some(v) => offset_needle = needle - v,
                    None => {
                        println!("Overflow sub Needle: {}, Offset: {}", needle, v);
                        continue;
                    }
                },
                Offset::Neg(v) => offset_needle = needle + v,
            };
            if offset_needle >= *key && offset_needle <= *key + range {
                return offset_needle;
            }
        }
        needle
    }

    fn get_seed(&self, location: usize, seeds: &Seeds) -> Option<usize> {
        let humidity = self.reverse_lookup(location, &self.location_map);
        let temperature = self.reverse_lookup(humidity, &self.humidity_map);
        let light = self.reverse_lookup(temperature, &self.temperature_map);
        let water = self.reverse_lookup(light, &self.light_map);
        let fertilizer = self.reverse_lookup(water, &self.water_map);
        let soil = self.reverse_lookup(fertilizer, &self.fertilizer_map);
        let seed = self.reverse_lookup(soil, &self.soil_map);
